...
export const voiceplayerI = async (interaction, query) => {
  ...
  queue.node.on('trackEnd', () => {
    const nextTrack = queue.tracks[1];  // Get the next track in the queue
    if (nextTrack) {
      console.log("Playing next track:", nextTrack.title);
      queue.addTrack(nextTrack);  // Add the next track to the queue
      queue.node.play();  // Play the next track
    }
  });
